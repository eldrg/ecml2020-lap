...
def train(model, optimizer, scheduler, criterion, trainloader, valloader, testloader, pnorm, lmt0, lml0, tau, uzs, nepochs, clip = 0):
	train_loss, train_acc1, train_acc5, val_loss, val_acc1, val_acc5, lmt, lml, t0, it = [], [], [], [], [], [], lmt0, lml0, time.time(), 0
	print('\n--- Begin trainning\n')
	for e in range(nepochs):
		...
		for j, (x, y) in enumerate(trainloader):
			...
			if uzs > 0 and tau > 0 and it % uzs == 0 :
...
def experiment(dataset, modelname, learnencoder, pnorm, nfilters, learningrate, lambdatransport, lambdaloss0, tau, uzawasteps, batchnorm, bias, timestep, clip, classifier,
	...
	print('experiment from all5.py with parameters')
	for name in names:
...
if __name__ == '__main__':
	...
	parser.add_argument("-dat", "--dataset", required = True, choices = ['cifar10', 'cifar100', 'imagenet2012', 'tinyimagenet', 'imagenetdownloader'], nargs = '*')
