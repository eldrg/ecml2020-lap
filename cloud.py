...
def makefolder(ndata, testsize, data, noise, factor, dataseed, modelseed, nblocks, datadim, hiddendim, batchnorm, nclasses, learnclassifier, 
	...
	folder0 = ('circles' if data == 1 else 'moons' if data == 2 else 'spirals') + '-dd' + str(datadim) + 'nc' + str(nclasses) 
	...
	folder = os.path.join(os.getcwd(), 'figures5', folder0, folder1, folder2, folder3, folder4, folder5, folder6, folder7, folder8, folder9)
	os.makedirs(folder)
	return folder

def dataloaders(ndata, testsize, data, noise, factor, dataseed, batchsize):
	if data == 1:
		X, Y = make_circles(ndata, True, noise, dataseed, factor)
	elif data == 2:
		X, Y = make_moons(ndata, True, noise, dataseed)
	elif data == 3:
		X, Y = make_spirals(int(ndata / 2), dataseed)
...
def experiment(ndata = 1000, 
	...
	print('--- experiment from cloud5.py with parameters')
	for name in names:
	...
	if data < 3:
		plot_border(X, Y, data, model, folder, batchnorm)
	if experiments:
		print('--- losses \n', losses, '\n--- accuracy \n', accuracy, '\n--- C \n', C, '\n--- W \n', W, '\n--- F \n', F)
		sys.stdout.close()
		sys.stdout = stdout0
...
def plot_border(X, Y, data, model, folder, batchnorm):
	model.eval()
	...
	n = 300
	M = np.zeros((n, n))
	xx, yy = np.linspace(-3, 3, n), np.linspace(-3, 3, n)
	...
	X = n * X / 4 + n / 2 if data == 1  else (n - 100) * X / 4 + (n - 1) / 2 if data == 2 else  20 * X / 4 + n / 2
	plt.scatter(X[Y == 0, 0], X[Y == 0, 1], c = 'red')
	plt.scatter(X[Y == 1, 0], X[Y == 1, 1], c = 'blue')
...
if __name__ == '__main__':
	...
	parser.add_argument("-da", "--data", type = int, default = [3], choices = [1, 2, 3], nargs = '*')
